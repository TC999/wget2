...
int main(void)
{
	memset(large_file, 'A', sizeof(large_file) / sizeof(char));
	large_file[sizeof(large_file) / sizeof(char) - 1] = '\0';

	memset(small_file, 'B', sizeof(small_file) / sizeof(char));
	small_file[sizeof(small_file) / sizeof(char) - 1] = '\0';
	...
	if (!WITHIN_RANGE(elapsed_ms, desired_ms, tolerance_ms)) {
		wget_error_printf_exit("Time taken for single file with limit-rate enabled "
		                       ...
		                       "(elapsed=%lli ms, desired=%lli±%lli ms)\n",
