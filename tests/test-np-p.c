 ...
 *
 *
 * Testing --page-requisites precedence over --no-parent
 *
 * Changelog
 * 04.05.2018  Tsukasa OI  created
 *
 */
...
int main(void)
{
	wget_test_url_t urls[]={
		...
		{	.name = "/firstlevel/fourthpage.html",
			...
			.body =
				"<html><head><title>Main Page</title></head><body><p>A link to a" \
				" <a href=\"/secondpage.html\">second page</a> and " \
		...
		{	.name = "/firstlevel/fifthpage.html",
			.code = "200 Dontcare",
			.body =
				"<html><head><title>Second Page</title></head><body><p>A link to a" \
				" <a href=\"http://localhost:{{port}}/thirdpage.html\">third page</a>." \
	...
	// --no-parent --page-requisites
	wget_test(
		WGET_TEST_OPTIONS, "--no-parent --page-requisites -nH",
		WGET_TEST_REQUEST_URL, "firstlevel/fourthpage.html",
		...
		WGET_TEST_EXPECTED_FILES, &(wget_test_file_t []) {
			...
			{ urls[10].name + 1, urls[10].body },   // test1.css
			{ urls[14].name + 1, urls[14].body }, // preload.css
			{	NULL } },
		0);

	// --no-parent --page-requisites --recursive
	wget_test(
		WGET_TEST_OPTIONS, "--no-parent --page-requisites -r -l 0 -nH",
		WGET_TEST_REQUEST_URL, "firstlevel/fourthpage.html",
		WGET_TEST_EXPECTED_ERROR_CODE, 0,
		WGET_TEST_EXPECTED_FILES, &(wget_test_file_t []) {
			{ urls[3].name + 1, urls[3].body },
			...
			{ urls[8].name + 1, urls[8].body },
			{ urls[10].name + 1, urls[10].body },   // test1.css
			{ urls[11].name + 1, urls[11].body },   // test2.css
			{ urls[12].name + 1, urls[12].body }, // myfavicon.ico
			{ urls[14].name + 1, urls[14].body }, // preload.css
			{	NULL } },
		0);
