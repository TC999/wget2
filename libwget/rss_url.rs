...
unsafe extern "C" fn rss_get_url(
    ...
    let mut ctx: *mut rss_context = context as *mut rss_context;
    let mut url: *mut wget_string = 0 as *mut wget_string;
    if val.is_null() || len == 0 {
        return;
    }
    if flags & (1 as libc::c_int) << 3 as libc::c_int != 0 {
        if wget_strcasecmp_ascii(attr, b"url\0" as *const u8 as *const libc::c_char) == 0
            || wget_strcasecmp_ascii(attr, b"href\0" as *const u8 as *const libc::c_char)
                == 0
        ...
        if !elem.is_null() {
            elem = elem.offset(1);
            elem;
            if wget_strcasecmp_ascii(elem, b"guid\0" as *const u8 as *const libc::c_char)
                == 0
                || wget_strcasecmp_ascii(
                    elem,
...
#[no_mangle]
pub unsafe extern "C" fn wget_rss_get_urls_inline(
    mut rss: *const libc::c_char,
    mut urls: *mut *mut wget_vector,
) {
    let mut context: rss_context = {
        let mut init = rss_context {
    ...
    wget_xml_parse_buffer(
        ...
        &mut context as *mut rss_context as *mut libc::c_void,
