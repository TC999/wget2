...
wget_tls_session *wget_tls_session_new(const char *host, int64_t maxage, const void *data, size_t data_size, const char *alpn)
{
	...
	if (tls_session) {
		...
		tls_session->alpn = wget_strdup(alpn);
...
int wget_tls_session_get(const wget_tls_session_db *tls_session_db, const char *host, void **data, size_t *size, char **alpn)
{
	if (tls_session_db) {
		...
		if (wget_hashmap_get(tls_session_db->entries, &tls_session, &tls_sessionp) && tls_sessionp->expires >= now) {
			...
			if (alpn)
				*alpn = wget_strdup(tls_sessionp->alpn);
			return 0;
		}
	}
...
static int tls_session_db_load(wget_tls_session_db *tls_session_db, FILE *fp)
{
	...
	while ((buflen = wget_getline(&buf, &bufsize, fp)) >= 0) {
		...
		if (*linep) {
			for (p = ++linep; *linep && !isspace(*linep); )
				linep++;
			tls_session.alpn = wget_strmemdup(p, linep - p);
		}
...
static int tls_session_save(void *_fp, const void *_tls_session, WGET_GCC_UNUSED void *v)
{
	...
	wget_fprintf(fp, "%s %lld %lld %s %s\n", tls_session->host, (long long)tls_session->created,
				 (long long)tls_session->maxage,
				 tls_session->alpn ? tls_session->alpn : "-",
				 session_b64);
...
static int tls_session_db_save(void *tls_session_db, FILE *fp)
{
	...
	if (wget_hashmap_size(entries) > 0) {
		...
		fputs("#<hostname> <created> <max-age> <alpn> <session data>\n\n", fp);
