...
#define HELP_FORMAT_FIRST          ("  %c%-4c  --%-16.16s  %s")
#define HELP_FORMAT_NEXT           ("  %5.5s  %-16.16s  %4.4s%s")

static inline void
print_first(const char s, const char *l, const char *msg)
{
	printf(HELP_FORMAT_FIRST, (s == 0 ? ' ' : '-'),
	                          (s == 0 ? ' ' : s) , l, msg);
}

static inline void
print_next(const char *msg)
{
	printf(HELP_FORMAT_NEXT, " ", " ", " ", msg);
}
