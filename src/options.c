...
static int run_use_askpass(char *question, char **answer)
{
	...
	*answer = (char *)wget_malloc(strlen(tmp)+1);
	memcpy(*answer, tmp, strlen(tmp)+1);
	return 0;
}
...
static int use_askpass(void)
{
	...
	if (config.username != NULL) xfree(config.username);
	/* Prompt for username */
	if (run_use_askpass(question, &config.username) < 0) return -1;
	snprintf(question, sizeof(question), "Type password for '%s':", config.username);
	if (config.password != NULL) xfree(config.password);
	/* Prompt for password */
	if (run_use_askpass(question, &config.password) < 0) return -1;
}
