...
unsafe fn main_0(
    ...
    req = wget_http_create_request(iri, b"POST\0" as *const u8 as *const libc::c_char);
    ...
    wget_http_request_set_body(
        ...
        wget_memdup((*body).data as *const libc::c_void, (*body).length)
            as *mut libc::c_char,
        (*body).length,
    );
    wget_http_open(&mut conn, iri);
    if !conn.is_null() {
            ...
            if !resp.is_null() {
                ...
                if !p.is_null() {
                    ...
                    if !e.is_null() {
                        ...
                        iri = wget_iri_parse((*url).data, 0 as *const libc::c_char);
                        req = wget_http_create_request(
                            iri,
                        ...
                        if !(wget_http_send_request(conn, req) != 0) {
                            ...
                            if !resp.is_null() {
                                ...
                                if !fp.is_null() {
                                    fwrite(
                                        (*(*resp).body).data as *const libc::c_void,
