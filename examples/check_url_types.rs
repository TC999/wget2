...
extern "C" {
    ...
    fn wget_dns_set_timeout(dns: *mut wget_dns, timeout: libc::c_int);
...
pub struct stats_t {
    ...
    pub http_links_same_host: libc::c_int,
    pub https_links_same_host: libc::c_int,
...
static mut stats: stats_t = stats_t {
    id: 0,
    http_links: 0,
    https_links: 0,
    http_links_same_host: 0,
    https_links_same_host: 0,
    status: 0,
    redirs: 0,
    redir_insecure: 0,
    landed_on_https: 0,
...
unsafe extern "C" fn write_stats() {
    ...
    if !fp.is_null() {
        fprintf(
            ...
            stats.http_links_same_host,
            stats.http_links,
            stats.https_links_same_host,
            stats.https_links,
        );
        fclose(fp);
    }
...
unsafe extern "C" fn html_parse(
    ...
    mut html_len: size_t,
    mut encoding: *const libc::c_char,
    mut hosturl: *const libc::c_char,
) {
    let mut base: *mut wget_iri = wget_iri_parse(
        hosturl,
        b"utf-8\0" as *const u8 as *const libc::c_char,
    );
    ...
    let mut sbuf: [libc::c_char; 1024] = [0; 1024];
    if *html.offset(0 as libc::c_int as isize) as libc::c_uchar as libc::c_int
        == 0xfe as libc::c_int
        && *html.offset(1 as libc::c_int as isize) as libc::c_uchar as libc::c_int
            == 0xff as libc::c_int
    {
        encoding = b"UTF-16BE\0" as *const u8 as *const libc::c_char;
        reason = b"set by BOM\0" as *const u8 as *const libc::c_char;
            ...
            == 0xbf as libc::c_int
    {
        ...
        reason = b"set by server response\0" as *const u8 as *const libc::c_char;
    }
    if wget_memiconv(
        encoding,
    ...
    {
        wget_info_printf(
            b"  Convert encoding '%s' (%s) to UTF-8\n\0" as *const u8
    ...
    if encoding.is_null() {
        if !((*parsed).encoding).is_null() {
            ...
            reason = b"set by document\0" as *const u8 as *const libc::c_char;
        } else {
            encoding = b"CP1252\0" as *const u8 as *const libc::c_char;
            reason = b"default, encoding not specified\0" as *const u8
                as *const libc::c_char;
        }
    }
    wget_info_printf(
    ...
    if !((*parsed).base.p).is_null() {
        ...
        if _normalize_uri(base, &mut (*parsed).base, encoding, &mut buf)
            == 0 as libc::c_int
        {
            if buf.length != 0 {
    ...
    while it < wget_vector_size((*parsed).uris) {
        let mut html_url: *mut wget_html_parsed_url = wget_vector_get((*parsed).uris, it)
            as *mut wget_html_parsed_url;
        let mut url: *mut wget_string = &mut (*html_url).url;
        if !(_normalize_uri(base, url, encoding, &mut buf) != 0
            || buf.length == 0 as libc::c_int as size_t)
        {
            let mut canon_url: *mut wget_iri = wget_iri_parse(
                buf.data,
                b"utf-8\0" as *const u8 as *const libc::c_char,
            );
            if !canon_url.is_null() {
                let mut same_host: libc::c_int = (wget_strcasecmp(
                    (*canon_url).host,
                    (*base).host,
                ) == 0) as libc::c_int;
                if (*canon_url).scheme as libc::c_uint
                    == WGET_IRI_SCHEME_HTTPS as libc::c_int as libc::c_uint
                {
                    stats.https_links += 1;
                    stats.https_links;
                    stats.https_links_same_host += same_host;
                } else if (*canon_url).scheme as libc::c_uint
    ...
    wget_info_printf(
        b"  same host: http=%d https=%d\n\0" as *const u8 as *const libc::c_char,
        stats.http_links_same_host,
        stats.https_links_same_host,
    );
    wget_info_printf(
        b"      total: http=%d https=%d\n\0" as *const u8 as *const libc::c_char,
        stats.http_links,
        stats.https_links,
    );
}
unsafe fn main_0(
    ...
    wget_tcp_set_timeout(0 as *mut wget_tcp, 5000 as libc::c_int);
    wget_tcp_set_connect_timeout(0 as *mut wget_tcp, 5000 as libc::c_int);
    wget_dns_set_timeout(0 as *mut wget_dns, 5000 as libc::c_int);
    wget_ssl_set_config_int(16 as libc::c_int, 0 as libc::c_int);
    wget_ssl_set_config_int(14 as libc::c_int, 0 as libc::c_int);
    while fscanf(
        stdin,
        b"%d,%255s\0" as *const u8 as *const libc::c_char,
        &mut stats.id as *mut libc::c_int,
        (stats.host).as_mut_ptr(),
    ) == 2 as libc::c_int
    {
        if !url.is_null() {
        ...
        {
            url = wget_aprintf(
                b"https://%s\0" as *const u8 as *const libc::c_char,
                (stats.host).as_mut_ptr().offset(7 as libc::c_int as isize),
            );
        ...
        stats.https_links_same_host = 0 as libc::c_int;
        ...
        let mut max: libc::c_int = 10 as libc::c_int;
        while redirs < max {
